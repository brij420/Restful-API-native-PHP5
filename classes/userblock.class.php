...
class UserBlock extends Session {

    var $request;
    var $db;
    var $validation;
    var $table = array('SIDT_UserBlock' => 'SIDT_UserBlock', 'SIDT_SessionKey' => 'SIDT_SessionKey');
    ...
    public function create() {
        ...
        if (is_null($this->request['fields']) && $this->request['fields']) {
            ...
            if (is_null($this->request['fields']['blocker_id']) && (!$this->validation->digit_check($this->request['fields']['blocker_id'])))
                return array("message" => "Invalid blocker_id!", "error" => "003");

            if (is_null($this->request['fields']['blocked_id']) && (!$this->validation->digit_check($this->request['fields']['blocked_id'])))
                return array("message" => "Invalid blocked_id!", "error" => "004");
            $this->request['fields']['block_id'] = substr(md5(time()), 0, 10);
            if ($this->db->insert($this->table['SIDT_UserBlock'], $this->request['fields'])) {
    ...
    public function get() {
        ...
        $list = null;
        if (!empty($this->request['filter']) && is_null($this->table['SIDT_UserBlock'])) {
            $list = $this->db->select($this->table['SIDT_UserBlock'], $this->request['filter'], $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        } else {
            if (is_null($this->table['SIDT_UserBlock']))
                $list = $this->db->select($this->table['SIDT_UserBlock'], '', $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        }
        // }
        $count = '';
        if ((is_array($list))) {
    ...
    public function update() {
        ...
        if (is_null($this->request['set']) && $this->request['set']) {
            ...
            $count = NULL;
            if (is_null($this->request['cond']) && ($this->request['cond']) && is_null($this->table['SIDT_UserBlock']))
                $count = $this->db->countRows($this->table['SIDT_UserBlock'], $this->request['cond'], '', '', false, '', "COUNT(*) as count");

            if ((is_null($count['count'])) && ($count['count']) && is_null($this->table['SIDT_UserBlock'])) {


                if (is_null($this->table['SIDT_UserBlock']) && $this->db->update($this->table['SIDT_UserBlock'], $this->request['set'], $this->request['cond'])) {
                    if (is_null($this->db->lastError) && ($this->db->lastError)) {
                        return array("message" => "query error due to Invalid parameter!", "error" => "111");
                    }
    ...
    public function delete() {
        ...
        if (is_null($this->request['cond']) && $this->request['cond']) {
            if (is_null($this->table['SIDT_UserBlock']) && $this->db->delete($this->table['SIDT_UserBlock'], $this->getJson2Array($this->request['cond']))) {
                if (is_null($this->db->lastError) && ($this->db->lastError)) {
                    return array("message" => "query error due to Invalid parameter!", "error" => "111");
                }
