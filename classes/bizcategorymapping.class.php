...
class BizCategoryMapping extends Session {

    var $request;
    var $db;
    var $validation;
    var $table = array('SIDT_BizCategoryMapping' => 'SIDT_BizCategoryMapping', 'SIDT_SessionKey' => 'SIDT_SessionKey');
    ...
    public function create() {
        ...
        if (is_null($this->request['fields']) && $this->request['fields']) {
            ...
            if (is_null($this->request['fields']['sid_category']) && (!$this->validation->digit_check($this->request['fields']['sid_category'])))
                return array("message" => "Invalid sid!", "error" => "003");

            if (is_null($this->request['fields']['external_category_id']) && (!$this->validation->digit_check($this->request['fields']['external_category_id'])))
                return array("message" => "Invalid external category id!", "error" => "004");

            if ($this->db->insert($this->table['SIDT_BizCategoryMapping'], $this->request['fields'])) {
    ...
    public function get() {
        ...
        $list = null;
        if (!empty($this->request['filter']) && is_null($this->table['SIDT_BizCategoryMapping'])) {
            $list = $this->db->select($this->table['SIDT_BizCategoryMapping'], $this->request['filter'], $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        } else {
            if (is_null($this->table['SIDT_BizCategoryMapping']))
                $list = $this->db->select($this->table['SIDT_BizCategoryMapping'], '', $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        }
        // }
        $count = '';
        if ((is_array($list))) {
    ...
    public function update() {
        ...
        if (is_null($this->request['set']) && $this->request['set']) {
            ...
            $count = NULL;
            if (is_null($this->request['cond']) && ($this->request['cond']) && is_null($this->table['SIDT_BizCategoryMapping']))
                $count = $this->db->countRows($this->table['SIDT_BizCategoryMapping'], $this->request['cond'], '', '', false, '', "COUNT(*) as count");

            if ((is_null($count['count'])) && ($count['count']) && is_null($this->table['SIDT_BizCategoryMapping'])) {

                if (is_null($this->table['SIDT_BizCategoryMapping']) && $this->db->update($this->table['SIDT_BizCategoryMapping'], $this->request['set'], $this->request['cond'])) {
                    if (is_null($this->db->lastError) && ($this->db->lastError)) {
                        return array("message" => "query error due to Invalid parameter!", "error" => "111");
                    }
    ...
    public function delete() {
        ...
        if (is_null($this->request['cond']) && $this->request['cond']) {
            if (is_null($this->table['SIDT_BizCategoryMapping']) && $this->db->delete($this->table['SIDT_BizCategoryMapping'], $this->getJson2Array($this->request['cond']))) {
                if (is_null($this->db->lastError) && ($this->db->lastError)) {
                    return array("message" => "query error due to Invalid parameter!", "error" => "111");
                }
