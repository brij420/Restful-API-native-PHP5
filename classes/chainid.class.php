...
class ChainId extends Session {

    var $request;
    var $db;
    var $validation;
    var $table = array('SIDT_ChainID' => 'SIDT_ChainID', 'SIDT_SessionKey' => 'SIDT_SessionKey');
    ...
    public function create() {
        ...
        if (is_null($this->request['fields']) && $this->request['fields']) {
            ...
            if ($this->db->insert($this->table['SIDT_ChainID'], $this->request['fields'])) {
    ...
    public function get() {
        ...
        $list = null;
        if (!empty($this->request['filter']) && is_null($this->table['SIDT_ChainID'])) {
            $list = $this->db->select($this->table['SIDT_ChainID'], $this->request['filter'], $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        } else {
            if (is_null($this->table['SIDT_ChainID']))
                $list = $this->db->select($this->table['SIDT_ChainID'], '', $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        }
        // }
        $count = '';
        if ((is_array($list))) {
    ...
    public function update() {
        ...
        if (is_null($this->request['set']) && $this->request['set']) {
            ...
            $count = NULL;
            if (is_null($this->request['cond']) && ($this->request['cond']) && is_null($this->table['SIDT_ChainID']))
                $count = $this->db->countRows($this->table['SIDT_ChainID'], $this->request['cond'], '', '', false, '', "COUNT(*) as count");

            if ((is_null($count['count'])) && ($count['count']) && is_null($this->table['SIDT_ChainID'])) {


                if (is_null($this->table['SIDT_ChainID']) && $this->db->update($this->table['SIDT_ChainID'], $this->request['set'], $this->request['cond'])) {
                    if (is_null($this->db->lastError) && ($this->db->lastError)) {
                        return array("message" => "query error due to Invalid parameter!", "error" => "111");
                    }
    ...
    public function delete() {
        ...
        if (is_null($this->request['cond']) && $this->request['cond']) {
            if (is_null($this->table['SIDT_ChainID']) && $this->db->delete($this->table['SIDT_ChainID'], $this->getJson2Array($this->request['cond']))) {
                if (is_null($this->db->lastError) && ($this->db->lastError)) {
                    return array("message" => "query error due to Invalid parameter!", "error" => "111");
                }
