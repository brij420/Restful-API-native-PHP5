...
class TickSheet extends Session {

    var $request;
    var $db;
    var $validation;
    var $table = array('SIDT_TickSheet' => 'SIDT_TickSheet', 'SIDT_SessionKey' => 'SIDT_SessionKey');
    ...
    public function create() {
        ...
        if (is_null($this->request['fields']) && $this->request['fields']) {
            ...
            if (is_null($this->request['fields']['school_id']) && (!$this->validation->digit_check($this->request['fields']['school_id'])))
                return array("message" => "Invalid school_id!", "error" => "003");

            if (is_null($this->request['fields']['biz_id']) && (!$this->validation->digit_check($this->request['fields']['biz_id'])))
                return array("message" => "Invalid biz_id!", "error" => "004");

            if ($this->db->insert($this->table['SIDT_TickSheet'], $this->request['fields'])) {
    ...
    public function get() {
        ...
        $list = null;
        if (!empty($this->request['filter']) && is_null($this->table['SIDT_TickSheet'])) {
            $list = $this->db->select($this->table['SIDT_TickSheet'], $this->request['filter'], $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        } else {
            if (is_null($this->table['SIDT_TickSheet']))
                $list = $this->db->select($this->table['SIDT_TickSheet'], '', $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        }
        // }
        $count = '';
        if ((is_array($list))) {
    ...
    public function update() {
        ...
        if (is_null($this->request['set']) && $this->request['set']) {
            ...
            $count = NULL;
            if (is_null($this->request['cond']) && ($this->request['cond']) && is_null($this->table['SIDT_TickSheet']))
                $count = $this->db->countRows($this->table['SIDT_TickSheet'], $this->request['cond'], '', '', false, '', "COUNT(*) as count");

            if ((is_null($count['count'])) && ($count['count']) && is_null($this->table['SIDT_TickSheet'])) {



                if (is_null($this->table['SIDT_TickSheet']) && $this->db->update($this->table['SIDT_TickSheet'], $this->request['set'], $this->request['cond'])) {
    ...
    public function delete() {
        ...
        if (is_null($this->request['cond']) && $this->request['cond']) {
            if (is_null($this->table['SIDT_TickSheet']) && $this->db->delete($this->table['SIDT_TickSheet'], $this->getJson2Array($this->request['cond']))) {
                if (is_null($this->db->lastError) && ($this->db->lastError)) {
                    return array("message" => "query error due to Invalid parameter!", "error" => "111");
                }
