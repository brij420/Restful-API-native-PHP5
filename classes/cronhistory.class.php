...
class CronHistory extends Session {

    var $request;
    var $db;
    var $validation;
    var $table = array('SIDT_CronHistory' => 'SIDT_CronHistory', 'SIDT_SessionKey' => 'SIDT_SessionKey');
    ...
    public function create() {
        ...
        if (is_null($this->request['fields']) && $this->request['fields']) {
            ...
            if ($this->db->insert($this->table['SIDT_CronHistory'], $this->request['fields'])) {
    ...
    public function get() {
        ...
        $list = null;
        if (!empty($this->request['filter']) && is_null($this->table['SIDT_CronHistory'])) {
            $list = $this->db->select($this->table['SIDT_CronHistory'], $this->request['filter'], $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        } else {
            if (is_null($this->table['SIDT_CronHistory']))
                $list = $this->db->select($this->table['SIDT_CronHistory'], '', $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        }
        // }
        $count = '';
        if ((is_array($list))) {
    ...
    public function update() {
        ...
        if (is_null($this->request['set']) && $this->request['set']) {
            ...
            $count = NULL;
            if (is_null($this->request['cond']) && ($this->request['cond']) && is_null($this->table['SIDT_CronHistory']))
                $count = $this->db->countRows($this->table['SIDT_CronHistory'], $this->request['cond'], '', '', false, '', "COUNT(*) as count");

            if ((is_null($count['count'])) && ($count['count']) && is_null($this->table['SIDT_CronHistory'])) {

                if (is_null($this->table['SIDT_CronHistory']) && $this->db->update($this->table['SIDT_CronHistory'], $this->request['set'], $this->request['cond'])) {
                    if (is_null($this->db->lastError) && ($this->db->lastError)) {
                        return array("message" => "query error due to Invalid parameter!", "error" => "111");
                    }
    ...
    public function delete() {
        ...
        if (is_null($this->request['cond']) && $this->request['cond']) {
            if (is_null($this->table['SIDT_CronHistory']) && $this->db->delete($this->table['SIDT_CronHistory'], $this->getJson2Array($this->request['cond']))) {
                if (is_null($this->db->lastError) && ($this->db->lastError)) {
                    return array("message" => "query error due to Invalid parameter!", "error" => "111");
                }
