...
class GroupMembers extends Session {

    var $request;
    var $db;
    var $validation;
    var $table = array('SIDT_GroupMembers' => 'SIDT_GroupMembers', 'SIDT_SessionKey' => 'SIDT_SessionKey');
    ...
    public function create() {
        ...
        if (is_null($this->request['fields']) && $this->request['fields']) {
            ...
            if (is_null($this->request['fields']['groupm_id']) && (!$this->validation->digit_check($this->request['fields']['groupm_id'])))
                return array("message" => "Invalid groupm_id!", "error" => "003");

            if (is_null($this->request['fields']['userm_id']) && (!$this->validation->digit_check($this->request['fields']['userm_id'])))
                return array("message" => "Invalid userm_id!", "error" => "004");

            if ($this->db->insert($this->table['SIDT_GroupMembers'], $this->request['fields'])) {
    ...
    public function get() {
        ...
        $list = null;
        if (!empty($this->request['filter']) && is_null($this->table['SIDT_GroupMembers'])) {
            $list = $this->db->select($this->table['SIDT_GroupMembers'], $this->request['filter'], $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        } else {
            if (is_null($this->table['SIDT_GroupMembers']))
                $list = $this->db->select($this->table['SIDT_GroupMembers'], '', $this->request['sortby'], $this->request['limit'], false, $this->request['conj'], $this->request['select'], $this->request['offset']);
        }
        // }
        $count = '';
        if ((is_array($list))) {
    ...
    public function update() {
        ...
        if (is_null($this->request['set']) && $this->request['set']) {
            ...
            $count = NULL;
            if (is_null($this->request['cond']) && ($this->request['cond']) && is_null($this->table['SIDT_GroupMembers']))
                $count = $this->db->countRows($this->table['SIDT_GroupMembers'], $this->request['cond'], '', '', false, '', "COUNT(*) as count");

            if ((is_null($count['count'])) && ($count['count']) && is_null($this->table['SIDT_GroupMembers'])) {

                if (is_null($this->table['SIDT_GroupMembers']) && $this->db->update($this->table['SIDT_GroupMembers'], $this->request['set'], $this->request['cond'])) {
                    if (is_null($this->db->lastError) && ($this->db->lastError)) {
                        return array("message" => "query error due to Invalid parameter!", "error" => "111");
                    }
    ...
    public function delete() {
        ...
        if (is_null($this->request['cond']) && $this->request['cond']) {
            if (is_null($this->table['SIDT_GroupMembers']) && $this->db->delete($this->table['SIDT_GroupMembers'], $this->getJson2Array($this->request['cond']))) {
                if (is_null($this->db->lastError) && ($this->db->lastError)) {
                    return array("message" => "query error due to Invalid parameter!", "error" => "111");
                }
